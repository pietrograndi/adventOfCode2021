...
const calcPreviousMeasurement = (items: number[]):number => {
  return items.reduce((acc:number,item:number, idx:number, array:number[]) => {
    if(idx === 0 ) return acc
...
const sumSlidingWindows = (src: number[], processed: number[] = []): [src: number[], processed: number[]] => {
...
const readCallback = (processCallback:(data:string) => void) => (err: Error, data:string) => {
  if(err) return
